...
http.createServer(function(request, response)
  ...
  //console_log("Request: " + uri);
     ...
     if (uri == "/poll")
      {
        var res =  pollResult();
        //   if (res.length > 0) console_log(res);
      ...
      else if (uri == "/reset_all")
      {
          motorsStop('all', "coast");
          response.writeHead(200);
          response.end();
          return;
      }
...
scratchRoutines =
{
    ...
    "setLightSensorMode" : setLightSensorMode,
    "setIRMode" : setIRMode,
...
var theLightSensorMode = "color";
function setLightSensorMode(mode)
{
    theLightSensorMode = mode;
}

var theIRMode = "measure distance";
function setIRMode(mode)
{
    theIRMode = mode;
}

var theMotorMode = "position";
function setMotorMode(mode)
{
    theMotorMode = mode;
}

var waitingScratchCommands = { };

var pollComm = "";
function dispatchCommand(parts)
{
    var foo = parts.join("/");
    var command = parts.shift();
    console_log("command: " + command);
    var routine = scratchRoutines[command];
    if (routine)
    {
        routine.apply(null, parts);
    }
    else
    {
        var wroutine = scratchWRoutines[command];
        if (wroutine)
        {
           // var commandID = parts.shift();
            console_log("commandID: " + command);
            waitingScratchCommands[command] = "busy";
            parts.push(function() { console_log("the callback!");
                       delete waitingScratchCommands[command]; });
            wroutine.apply(null, parts);
        }
        else
        {
            
            /*
            var Rroutine = scratchSensors[command];
            if (Rroutine)
            {
                parts.push(function(result) { console_log("the sensor callback");
                           pollComm += foo + " " + result + "\r\n";
                           });
                Rroutine.apply(null, parts);
            }
           */
        }
    }
...
function pollResult()
{
    var res = "";
    if (!notConnected())
        res += "_success Connected\n";
    else
        res += "_problem Not connected\n";
    ...
    var busy = "";
    for (com in waitingScratchCommands)
    {
        busy += com + " ";
    }
    if (busy.length)
    ...
    {
        for (var i = 0; i < port_Assignments.length; i++)
        {
            var port = port_Assignments[i];
            var commandName = null;
            if (port == '1E' || port == '21')
            {
                if (port == '21' && theIRMode == "remote control")
                    commandName = "readRemoteButtonPort";
                else
                    commandName = "readDistanceSensorPort";
            }
            else if (port == '10' || port == '01')
            {
                commandName = "readTouchSensorPort";
            }
            else if (port == '07' || port == '08')
            {
                if (theMotorMode == "position")
                    commandName = "readMotorPosition";
                else if (theMotorMode == "speed")
                    commandName = "readMotorSpeed";
            }
            else if (port == '1D' || port == '04')
            {
                commandName = "readColorSensorPort";
            }
            if (commandName)
                res += commandName + "/" + sensorPortsNames[i] + " " + encodeURIComponent(global_sensor_result[i]) + "\n";
        }
    }
...
console_log("Static file server running at\n  => http://localhost:" + port + "/\nCTRL + C to shutdown");

var SerialPort = require("serialport");

var potentialEV3Devices = [];
var EV3Devices = [];
var theEV3Device = "";
...
SerialPort.list(function (err, ports) {
              ...
              console_log(port.comName);
            
              if (port.comName.indexOf('-SerialPort') != -1 || port.comName.indexOf('COM') === 0)
              {
                 potentialEV3Devices.push(port.comName);
              }

              
              });
                    deviceTimeout = setTimeout(tryAllDevices, 10);
...
{
    EV3Devices = potentialEV3Devices.slice(); // make copy by value
    if (potentialEV3Devices.length == 0)
        console_log("No devices found.");
    else
        tryNextDevice();
}
...
{
    EV3Devices.sort((function(a, b){return b.localeCompare(a)}));
    
    var device = EV3Devices.shift();
...
function weConnected()
{
    ...
    if (!scanInterval)
        setTimeout( function() { scanInterval = setInterval(startScanning, 500); }, 3000);
}

function startScanning()
{
//    console_log("scanning ports");
    scanPorts(function()
              {
      ...
      for (var i = 0; i < port_Assignments.length; i++)
        {
            var port = port_Assignments[i];
              var pName = sensorPortsNames[i];
            //  console_log(port + "(" + pName + ")");
            if (port == '1E' || port == '21')
            {
              if (port == '21' && theIRMode == "remote control")
                    readRemoteButtonPort(pName, function(result) {   })
              else
                    readDistanceSensorPort(pName, function(result) {   })
              
            }
            else  if (port == '10' || port == '01')
            {
                readTouchSensorPort(pName, function(result) {   })
            }
            else  if (port == '1D'|| port == '04')
            {
                readColorSensorPort(pName, theLightSensorMode, function(result) {   })
            }
            else  if (port == '07'|| port == '08')
            {
                readFromMotor(theMotorMode, pName, function(result) {   })
            }
        }
    });
}
...
function tryToConnect()
{
    ...
    serialport = new SerialPort.SerialPort(theEV3Device, {
    ...
    serialport.open( function(error){
                  if (error)
                  {
                      console_log("Serial Port " + theEV3Device + "  Error: "  + error);
                  }
                  else
                  {
                      console_log("Serial Port " + theEV3Device + " Opened");
...
function sendCommand(commandArray)
{
    if ((EV3Connected || connecting) && theEV3Device)
    {
        ...
        if (serialport.isOpen())
            serialport.write(commandArray.buffer);
    }
    else
    {
       ...
       // console_log("INPUT_DEVICE_GET_TYPE_MODE");
        ...
        for (x = 0; x <= 7; x++)
        {
            ...
            if (val)
            {
                ...
                port_Assignments[x] = hexcouplet(val);
            }
        }
    }
...
function scanPorts(callback)
{
    ...
    addToQueryQueue([9, INPUT_DEVICE_GET_TYPE_MODE, 0, callback, theCommand]);
}
